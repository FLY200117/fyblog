<template><div><h1 id="进阶目录" tabindex="-1"><a class="header-anchor" href="#进阶目录" aria-hidden="true">#</a> 进阶目录</h1>
<nav class="table-of-contents"><ul><li><router-link to="#reconciliation">Reconciliation</router-link><ul><li><router-link to="#设计动机">设计动机</router-link></li><li><router-link to="#diffing算法">Diffing算法</router-link></li></ul></li><li><router-link to="#fiber">Fiber</router-link><ul><li><router-link to="#fiber的重要型">Fiber的重要型</router-link></li><li><router-link to="#什么是fiber">什么是Fiber</router-link></li><li><router-link to="#requestanimationframe-绘制动画api">requestAnimationFrame（绘制动画API）</router-link></li><li><router-link to="#requestidlecallback-后台执行api">requestIdleCallback（后台执行API）</router-link></li><li><router-link to="#fiber链表结构设计">Fiber链表结构设计</router-link></li><li><router-link to="#fiber节点设计">Fiber节点设计</router-link></li><li><router-link to="#fiber执行原理">Fiber执行原理</router-link></li></ul></li></ul></nav>
<p>在了解Fiber之前，需要知道的是现在流行的VDOM（Virtual DOM），以下是官网给出的对VDOM的解释：Virtual DOM 是一种编程概念。在这个概念里， UI 以一种理想化的，或者说“虚拟的”表现形式被保存于内存中，并通过如 ReactDOM 等类库使之与“真实的” DOM 同步。这一过程叫做<a href="https://zh-hans.reactjs.org/docs/reconciliation.html" target="_blank" rel="noopener noreferrer">协调<ExternalLinkIcon/></a>。</p>
<p>Vue和其他主流框架底层都是运用了VDOM，原因是因为浏览器底层对DOM的直接操作过于浪费，所以将真实DOM映射成下面这种形式：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过这样的&quot;虚拟&quot;DOM去<code v-pre>统一</code>的操作DOM，在这基础上，还增加了<code v-pre>diff</code>算法，这里就不过多讲述<code v-pre>diff</code>算法，由VDOM这一技术陆陆续续又衍生了许多其他技术，这里就不过多讲解。</p>
<p>注意：<strong>Shadow DOM和VDOM并不一样</strong>，前者是一种浏览器技术，主要是在web组件中封装变量和CSS，而VDOM则更像是JS库基于浏览器API实现的概念，<strong>与其将 “Virtual DOM” 视为一种技术，不如说它是一种模式</strong></p>
<h2 id="reconciliation" tabindex="-1"><a class="header-anchor" href="#reconciliation" aria-hidden="true">#</a> Reconciliation</h2>
<h3 id="设计动机" tabindex="-1"><a class="header-anchor" href="#设计动机" aria-hidden="true">#</a> 设计动机</h3>
<p>在某一时间节点调用 React 的 <code v-pre>render()</code> 方法，会创建一棵由 React 元素组成的树。在下一次 state 或 props 更新时，相同的 <code v-pre>render()</code> 方法会返回一棵不同的树。React 需要基于这两棵树之间的差别来判断如何高效的更新 UI，以保证当前 UI 与最新的树保持同步</p>
<p>此算法有一些通用的解决方案，即生成将一棵树转换成另一棵树的最小操作次数。然而，即使使用<a href="http://grfia.dlsi.ua.es/ml/algorithms/references/editsurvey_bille.pdf" target="_blank" rel="noopener noreferrer">最优的算法<ExternalLinkIcon/></a>，该算法的复杂程度仍为 O(n 3 )，其中 n 是树中元素的数量</p>
<p>如果在 React 中使用该算法，那么展示 1000 个元素则需要 10 亿次的比较。这个开销实在是太过高昂。于是 React 在以下两个假设的基础之上提出了一套 O(n) 的启发式算法：</p>
<ol>
<li>两个不同类型的元素会产生出不同的树；</li>
<li>开发者可以使用 <code v-pre>key</code> 属性标识哪些子元素在不同的渲染中可能是不变的。</li>
</ol>
<p>在实践中，我们发现以上假设在几乎所有实用的场景下都成立。</p>
<h3 id="diffing算法" tabindex="-1"><a class="header-anchor" href="#diffing算法" aria-hidden="true">#</a> Diffing算法</h3>
<p>当对比两棵树时，React首先比较两棵树的根节点。不同类型的根节点元素会有不同的形态</p>
<h4 id="对比不同类型的元素" tabindex="-1"><a class="header-anchor" href="#对比不同类型的元素" aria-hidden="true">#</a> 对比不同类型的元素</h4>
<p>当根节点为不同类型的元素时，React 会拆卸原有的树并且建立起新的树。举个例子，当一个元素从 <code v-pre>&lt;a&gt;</code> 变成 <code v-pre>&lt;img&gt;</code>，从 <code v-pre>&lt;Article&gt;</code> 变成 <code v-pre>&lt;Comment&gt;</code>，或从 <code v-pre>&lt;Button&gt;</code> 变成 <code v-pre>&lt;div&gt;</code> 都会触发一个完整的重建流程</p>
<p>当卸载一棵树时，对应的 DOM 节点也会被销毁。组件实例将执行 <code v-pre>componentWillUnmount()</code> 方法。当建立一棵新的树时，对应的 DOM 节点会被创建以及插入到 DOM 中。组件实例将执行 <code v-pre>UNSAFE_componentWillMount()</code> 方法，紧接着 <code v-pre>componentDidMount()</code> 方法。所有与之前的树相关联的 state 也会被销毁</p>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>span</span><span class="token punctuation">></span></span>
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>Counter</span> <span class="token punctuation">/></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>span</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>React 会销毁 <code v-pre>Counter</code> 组件并且重新装载一个新的组件。</p>
<h4 id="对比同一类型的元素" tabindex="-1"><a class="header-anchor" href="#对比同一类型的元素" aria-hidden="true">#</a> 对比同一类型的元素</h4>
<p>当对比两个相同类型的 React 元素时，React 会保留 DOM 节点，仅比对及更新有改变的属性。比如：</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>div</span> <span class="token attr-name">className</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>before<span class="token punctuation">"</span></span> <span class="token attr-name">title</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>stuff<span class="token punctuation">"</span></span> <span class="token punctuation">/></span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过对比这两个元素，React 知道只需要修改 DOM 元素上的 <code v-pre>className</code> 属性。</p>
<p>当更新 <code v-pre>style</code> 属性时，React 仅更新有所更变的属性。比如：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过对比这两个元素，React 知道只需要修改 DOM 元素上的 <code v-pre>color</code> 样式，无需修改 <code v-pre>fontWeight</code>。</p>
<p>在处理完当前节点之后，React 继续对子节点进行递归。</p>
<h4 id="对比同类型的组件元素" tabindex="-1"><a class="header-anchor" href="#对比同类型的组件元素" aria-hidden="true">#</a> 对比同类型的组件元素</h4>
<p>当一个组件更新时，组件实例会保持不变，因此可以在不同的渲染时保持 state 一致。React 将更新该组件实例的 props 以保证与最新的元素保持一致，并且调用该实例的 <code v-pre>UNSAFE_componentWillReceiveProps()</code>、<code v-pre>UNSAFE_componentWillUpdate()</code> 以及 <code v-pre>componentDidUpdate()</code> 方法。</p>
<p>下一步，调用 <code v-pre>render()</code> 方法，diff 算法将在之前的结果以及新的结果中进行递归。</p>
<h4 id="对子节点进行递归" tabindex="-1"><a class="header-anchor" href="#对子节点进行递归" aria-hidden="true">#</a> 对子节点进行递归</h4>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>ul</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>React 会先匹配两个 <code v-pre>&lt;li&gt;first&lt;/li&gt;</code> 对应的树，然后匹配第二个元素 <code v-pre>&lt;li&gt;second&lt;/li&gt;</code> 对应的树，最后插入第三个元素的 <code v-pre>&lt;li&gt;third&lt;/li&gt;</code> 树</p>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>ul</span><span class="token punctuation">></span></span>
  ...
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>li</span><span class="token punctuation">></span></span>Villanova<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>li</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>ul</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>React 并不会意识到应该保留 <code v-pre>&lt;li&gt;Duke&lt;/li&gt;</code> 和 <code v-pre>&lt;li&gt;Villanova&lt;/li&gt;</code>，而是会重建每一个子元素。这种情况会带来性能问题</p>
<h4 id="keys" tabindex="-1"><a class="header-anchor" href="#keys" aria-hidden="true">#</a> Keys</h4>
<p>为了解决上述问题，React 引入了 <code v-pre>key</code> 属性。当子元素拥有 key 时，React 使用 key 来匹配原有树上的子元素以及最新树上的子元素。以下示例在新增 <code v-pre>key</code> 之后，使得树的转换效率得以提高：</p>
<div class="language-html ext-html line-numbers-mode"><pre v-pre class="language-html"><code><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>ul</span><span class="token punctuation">></span></span>
...
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>ul</span><span class="token punctuation">></span></span>
  ...
  <span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>li</span> <span class="token attr-name">key</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>2016<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>Villanova<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>li</span><span class="token punctuation">></span></span>
<span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>ul</span><span class="token punctuation">></span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>现在 React 知道只有带着 <code v-pre>'2014'</code> key 的元素是新元素，带着 <code v-pre>'2015'</code> 以及 <code v-pre>'2016'</code> key 的元素仅仅移动了</p>
<p>可以新增一个 ID 字段到模型中，或者利用一部分内容作为哈希值来生成一个 key。这个 key 不需要全局唯一，但在<strong>列表</strong>中需要保持唯一</p>
...
<p>Fiber 是 React 16 中新的协调引擎。它的主要目的是使 Virtual DOM 可以进行增量式渲染。Fiber 是对 React 核心算法的重构，facebook 团队使用两年多的时间去重构 React 的核心算法，在React16 以上的版本中引入了 Fiber 架构</p>
<h3 id="fiber的重要型" tabindex="-1"><a class="header-anchor" href="#fiber的重要型" aria-hidden="true">#</a> Fiber的重要型</h3>
<p>在浏览器中，页面是一帧帧绘制出来的，渲染的帧率与设备的刷新率保持一致，一般情况下是1s 60次，当每秒内绘制的帧数（FPS）超过60时，页面渲染是流畅的，当小于60的时候则会出现一定的卡顿现象</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4f288e9135ed414d871ad2fd2715d85d~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<ol>
<li>首先需要处理输入时间，能让用户得到最早的反馈</li>
<li>处理定时器，需要检查定时器是否到时间，并执行对应的回调</li>
<li>接下来处理Begin Frame（开始帧），即每一帧的时间，包括 window.resize、scroll、media query change 等</li>
...
<p>js引擎和页面渲染引擎是在同一个渲染线程之内，两者是互斥关系。如果在某个阶段执行任务特别长，例如在定时器阶段或<code v-pre>Begin Frame</code>阶段执行时间非常长，时间已经明显超过了16ms，那么就会阻塞页面的渲染，从而出现卡顿现象。</p>
<p>在 react16 引入 Fiber 架构之前，react 会采用先序递归对比虚拟DOM树，找出需要变动的节点，然后同步更新它们，这个过程 react 称为<code v-pre>reconcilation</code>（协调）。在<code v-pre>reconcilation</code>期间，react 会一直占用浏览器资源，会导致用户触发的事件得不到响应。实现的原理如下所示</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/994d70ab12f14d079e6c0b98d0e1b326~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<p>这种遍历是递归调用，执行栈会越来越深，而且不能中断，中断后就不能恢复了。递归如果非常深，就会十分卡顿。如果递归花了100ms，则这100ms浏览器是无法响应的，代码执行时间越长卡顿越明显。传统的方法存在不能中断和执行栈太深的问题。</p>
<p>因此，为了解决以上的痛点问题，React希望能够彻底解决主线程长时间占用问题，于是引入了 Fiber 来改变这种不可控的现状，把渲染/更新过程拆分为一个个小块的任务，通过合理的调度机制来调控时间，指定任务执行的时机，从而降低页面卡顿的概率，提升页面交互体验。通过Fiber架构，让<code v-pre>reconcilation</code>过程变得可被中断。适时地让出CPU执行权，可以让浏览器及时地响应用户的交互。</p>
...
<h4 id="执行单位" tabindex="-1"><a class="header-anchor" href="#执行单位" aria-hidden="true">#</a> 执行单位</h4>
<p>Fiber 可以理解为一个执行单元，每次执行完一个执行单元，react 就会检查现在还剩多少时间，如果没有时间则将控制权让出去。React Fiber 与浏览器的核心交互流程如下：</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/78a602cbc87342628ace49abb5d20c39~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
...
<h4 id="数据结构" tabindex="-1"><a class="header-anchor" href="#数据结构" aria-hidden="true">#</a> 数据结构</h4>
<p>Fiber 还可以理解为是一种数据结构，React Fiber 就是采用链表实现的。每个 Virtual DOM 都可以表示为一个 fiber，如下图所示，每个节点都是一个 fiber。一个 fiber包括了 child（第一个子节点）、sibling（兄弟节点）、return（父节点）等属性，React Fiber 机制的实现，就是依赖于以下的数据结构</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/74fe5e7d1dc449568448b462abcbff6e~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<h3 id="requestanimationframe-绘制动画api" tabindex="-1"><a class="header-anchor" href="#requestanimationframe-绘制动画api" aria-hidden="true">#</a> requestAnimationFrame（绘制动画API）</h3>
<p>在 Fiber 中使用到了<a href="https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fzh-CN%2Fdocs%2FWeb%2FAPI%2FWindow%2FrequestAnimationFrame" target="_blank" rel="noopener noreferrer">requestAnimationFrame<ExternalLinkIcon/></a>，它是浏览器提供的绘制动画的 api 。它要求浏览器在下次重绘之前（即下一帧）调用指定的回调函数更新动画</p>
<h3 id="requestidlecallback-后台执行api" tabindex="-1"><a class="header-anchor" href="#requestidlecallback-后台执行api" aria-hidden="true">#</a> requestIdleCallback（后台执行API）</h3>
<p><a href="https://link.juejin.cn?target=https%3A%2F%2Fdeveloper.mozilla.org%2Fzh-CN%2Fdocs%2FWeb%2FAPI%2FWindow%2FrequestIdleCallback" target="_blank" rel="noopener noreferrer">requestIdleCallback<ExternalLinkIcon/></a> 也是 react Fiber 实现的基础 api 。我们希望能够快速响应用户，让用户觉得够快，不能阻塞用户的交互，<code v-pre>requestIdleCallback</code>能使开发者在主事件循环上执行后台和低优先级的工作，而不影响延迟关键事件，如动画和输入响应。正常帧任务完成后没超过16ms，说明有多余的空闲时间，此时就会执行<code v-pre>requestIdleCallback</code>里注册的任务</p>
<p>具体的执行流程如下，开发者采用<code v-pre>requestIdleCallback</code>方法注册对应的任务，告诉浏览器我的这个任务优先级不高，如果每一帧内存在空闲时间，就可以执行注册的这个任务。另外，开发者是可以传入<code v-pre>timeout</code>参数去定义超时时间的，如果到了超时时间了，浏览器必须立即执行，使用方法如下：<code v-pre>window.requestIdleCallback(callback, { timeout: 1000 })</code>。浏览器执行完这个方法后，如果没有剩余时间了，或者已经没有下一个可执行的任务了，React应该归还控制权，并同样使用<code v-pre>requestIdleCallback</code>去申请下一个时间片。具体的流程如下图：</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/043bc29786cd4053a056f95d3256cb2e~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<p><code v-pre>window.requestIdleCallback(callback)</code>的<code v-pre>callback</code>中会接收到默认参数 deadline ，其中包含了以下两个属性：</p>
<ul>
<li>timeRamining 返回当前帧还剩多少时间供用户使用</li>
<li>didTimeout 返回 callback 任务是否超时</li>
</ul>
<p>这里需要注意的是，浏览器一帧的严格时间并不是16ms，是可以动态控制的，如果子任务时间超过了一帧的剩余时间，则会一直卡在这里执行，直到代码执行完毕。如果代码存在死循环，则浏览器会卡死，如果此帧的剩余时间大于0（有空余时间）或者已经超时，且当时存在任务，则直接执行该任务。如果没有剩余时间，则应该放弃执行任务控制权，把执行权交还给浏览器。如果多个任务执行总时间小于空闲时间的话，是可以在一帧内执行多个任务的。</p>
<h3 id="fiber链表结构设计" tabindex="-1"><a class="header-anchor" href="#fiber链表结构设计" aria-hidden="true">#</a> Fiber链表结构设计</h3>
<p>Fiber结构是使用链表实现的，<code v-pre>Fiber tree</code>实际上是个单链表树结构，详见<a href="https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Ffacebook%2Freact%2Fblob%2Fmaster%2Fpackages%2Freact-reconciler%2Fsrc%2FReactFiber.new.js" target="_blank" rel="noopener noreferrer">ReactFiber.js源码<ExternalLinkIcon/></a>，在这里我们看看Fiber的链表结构是怎样的，了解了这个链表结构后，能更快地理解后续 Fiber 的遍历过程。</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b440a981e4c040b2ab3523f065bffea7~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<p>以上每一个单元包含了<code v-pre>payload</code>（数据）和<code v-pre>nextUpdate</code>（指向下一个单元的指针），定义结构如下：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">Update</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来定义一个队列，把每个单元串联起来，其中定义了两个指针：头指针<code v-pre>firstUpdate</code>和尾指针<code v-pre>lastUpdate</code>，作用是指向第一个单元和最后一个单元，并加入了<code v-pre>baseState</code>属性存储React中的state状态。如下所示：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">UpdateQueue</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来定义两个方法：插入节点单元（enqueueUpdate）、更新队列（forceUpdate）。插入节点单元时需要考虑是否已经存在节点，如果不存在直接将<code v-pre>firstUpdate</code>、<code v-pre>lastUpdate</code>指向此节点即可。更新队列是遍历这个链表，根据<code v-pre>payload</code>中的内容去更新<code v-pre>state</code>的值。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">class</span> <span class="token class-name">UpdateQueue</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="fiber节点设计" tabindex="-1"><a class="header-anchor" href="#fiber节点设计" aria-hidden="true">#</a> Fiber节点设计</h3>
<p>Fiber的拆分单位是fiber（<code v-pre>fiber tree</code>上的一个节点），实际上就是按虚拟DOM节点拆，我们需要根据虚拟dom去生成 Fiber 树。下文中我们把每一个节点叫做 fiber 。fiber 节点结构如下</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token punctuation">{</span>
    ...
    <span class="token comment">// ……     </span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>fiber节点包括了以下属性：</p>
<ol>
<li>type &amp; key</li>
</ol>
<ul>
<li>fiber的type和key与React元素的作用相同。fiber的type描述了它对应的组件，对于复合组件，type是函数或者类组件本身。对于原生标签，type是一个字符串。随着type的不同，在reconciliation期间使用key来确定fiber是否可以重新使用</li>
</ul>
<ol start="2">
<li>stateNode</li>
</ol>
...
<ol start="3">
<li>child &amp; sibling &amp; return</li>
</ol>
<ul>
<li>
<h6 id="child-属性指向此节点的第一个子节点-大儿子-。" tabindex="-1"><a class="header-anchor" href="#child-属性指向此节点的第一个子节点-大儿子-。" aria-hidden="true">#</a> child 属性指向此节点的第一个子节点（大儿子）。</h6>
</li>
<li>
<p>sibling 属性指向此节点的下一个兄弟节点（大儿子指向二儿子、二儿子指向三儿子）</p>
</li>
<li>
<p>return 属性指向此节点的父节点，即当前节点处理完毕后，应该向谁提交自己的成果。如果 fiber 具有多个子 fiber，则每个子 fiber 的 return fiber 是 parent</p>
</li>
</ul>
<p>所有 fiber 节点都通过以下属性：child，sibling 和 return来构成一个 fiber node 的 linked list(后面称之为链表)。如下图所示：</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5192251a0a47481aacb6e018b1741655~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<p>其他的属性还有<code v-pre>memoizedState</code>（创建输出的 fiber 的状态）、<code v-pre>pendingProps</code>（将要改变的  props ）、<code v-pre>memoizedProps</code>（上次渲染创建输出的 props ）、<code v-pre>pendingWorkPriority</code>（定义 fiber 工作优先级）等等</p>
<h3 id="fiber执行原理" tabindex="-1"><a class="header-anchor" href="#fiber执行原理" aria-hidden="true">#</a> Fiber执行原理</h3>
<p>从根节点开始渲染和调度的过程可以分为两个阶段：render阶段，commit阶段</p>
<ul>
<li>render阶段：该阶段是可中断的，会找出所有节点的变更</li>
<li>commit阶段：该阶段是不可中断的，会执行所有的变更</li>
</ul>
<h4 id="render阶段" tabindex="-1"><a class="header-anchor" href="#render阶段" aria-hidden="true">#</a> render阶段</h4>
<p>此阶段会找出所有节点的变更，如节点新增、删除、属性变更等，这些变更 react 统称为副作用（effect），此阶段会构建一棵<code v-pre>Fiber tree</code>，以虚拟dom节点为维度对任务进行拆分，即一个虚拟dom节点对应一个任务，最后产出的结果是<code v-pre>effect list</code>，从中可以知道哪些节点更新、哪些节点增加、哪些节点删除了</p>
<h5 id="遍历流程" tabindex="-1"><a class="header-anchor" href="#遍历流程" aria-hidden="true">#</a> 遍历流程</h5>
<p><code v-pre>React Fiber</code>首先是将虚拟DOM树转化为<code v-pre>Fiber tree</code>，因此每个节点都有<code v-pre>child</code>、<code v-pre>sibling</code>、<code v-pre>return</code>属性，遍历<code v-pre>Fiber tree</code>时采用的是后序遍历方法：</p>
<ol>
<li>如果有大儿子，先遍历大儿子；如果没有大儿子，则表示遍历完成</li>
<li>大儿子： a. 如果有弟弟，则返回弟弟，跳到2 b. 如果没有弟弟，则返回父节点，并标志完成父节点遍历，跳到2 d. 如果没有父节点则标志遍历结束</li>
<li>遍历完所有节点后，自己才完成</li>
</ol>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3e617a3507074e498318332b579cd634~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<h5 id="收集effect-list" tabindex="-1"><a class="header-anchor" href="#收集effect-list" aria-hidden="true">#</a> 收集effect list</h5>
<p>知道了遍历方法之后，接下来需要做的工作就是在遍历过程中，收集所有节点的变更产出<code v-pre>effect list</code>，注意其中只包含了需要变更的节点。通过每个节点更新结束时向上归并<code v-pre>effect list</code>来收集任务结果，最后根节点的<code v-pre>effect list</code>里就记录了包括了所有需要变更的结果。</p>
<p>收集<code v-pre>effect list</code>的具体步骤为：</p>
<ol>
<li>
<p>如果当前节点需要更新，则打<code v-pre>tag</code>更新当前节点状态（props, state, context等）</p>
</li>
<li>
<p>为每个子节点创建fiber。如果没有产生<code v-pre>child fiber</code>，则结束该节点，把<code v-pre>effect list</code>归并到<code v-pre>return</code>，把此节点的<code v-pre>sibling</code>节点作为下一个遍历节点；否则把<code v-pre>child</code>节点作为下一个遍历节点</p>
</li>
<li>
<p>如果有剩余时间，则开始下一个节点，否则等下一次主线程空闲再开始下一个节点</p>
</li>
<li>
<p>如果没有下一个节点了，进入<code v-pre>pendingCommit</code>状态，此时<code v-pre>effect list</code>收集完毕，结束。</p>
</li>
</ol>
<p>收集<code v-pre>effect list</code>的遍历顺序如下所示：</p>
<p><img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/36484ed7dca5465695c99f7fe329ed6d~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp" alt=""></p>
<p>遍历子虚拟DOM元素数组，为每个虚拟DOM元素创建子fiber：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> <span class="token function-variable function">reconcileChildren</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter">currentFiber<span class="token punctuation">,</span> newChildren</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>定义一个方法收集此 fiber 节点下所有的副作用，并组成<code v-pre>effect list</code>。注意每个 fiber 有两个属性：</p>
<ul>
<li>firstEffect：指向第一个有副作用的子fiber</li>
<li>lastEffect：指向最后一个有副作用的子fiber</li>
</ul>
<p>中间的使用<code v-pre>nextEffect</code>做成一个单链表。</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 在完成的时候要收集有副作用的fiber，组成effect list</span>
<span class="token keyword">const</span> <span class="token function-variable function">completeUnitOfWork</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter">currentFiber</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来定义一个递归函数，从根节点出发，把全部的 fiber 节点遍历一遍，产出最终全部的<code v-pre>effect list</code>：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token comment">// 把该节点和子节点任务都执行完</span>
<span class="token keyword">const</span> <span class="token function-variable function">performUnitOfWork</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter">currentFiber</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="commit阶段" tabindex="-1"><a class="header-anchor" href="#commit阶段" aria-hidden="true">#</a> commit阶段</h4>
<p>commit 阶段需要将上阶段计算出来的需要处理的副作用一次性执行，此阶段不能暂停，否则会出现UI更新不连续的现象。此阶段需要根据<code v-pre>effect list</code>，将所有更新都 commit 到DOM树上。</p>
<h5 id="根据一个-fiber-的-effect-list-更新视图" tabindex="-1"><a class="header-anchor" href="#根据一个-fiber-的-effect-list-更新视图" aria-hidden="true">#</a> 根据一个 fiber 的 effect list 更新视图</h5>
<p>根据一个 fiber 的<code v-pre>effect list</code>列表去更新视图（这里只列举了新增节点、删除节点、更新节点的三种操作）：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> <span class="token function-variable function">commitWork</span> <span class="token operator">=</span> <span class="token parameter">currentFiber</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="根据全部fiber的effect-list-更新视图" tabindex="-1"><a class="header-anchor" href="#根据全部fiber的effect-list-更新视图" aria-hidden="true">#</a> 根据全部fiber的effect list 更新视图</h5>
<p>写一个递归函数，从根节点出发，根据<code v-pre>effect list</code>完成全部更新：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> <span class="token function-variable function">commitRoot</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="完成视图更新" tabindex="-1"><a class="header-anchor" href="#完成视图更新" aria-hidden="true">#</a> 完成视图更新</h5>
<p>接下来定义循环执行工作，当计算完成每个 fiber 的<code v-pre>effect list</code>后，调用 commitRoot 完成视图更新：</p>
<div class="language-javascript ext-js line-numbers-mode"><pre v-pre class="language-javascript"><code><span class="token keyword">const</span> <span class="token function-variable function">workloop</span> <span class="token operator">=</span> <span class="token punctuation">(</span><span class="token parameter">deadline</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>到这时，已经根据收集到的变更信息，完成了视图的刷新操作。</p>
</div></template>
